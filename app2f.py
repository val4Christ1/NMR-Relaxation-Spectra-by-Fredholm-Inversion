...
def extract_tissue_signal(volumes_list, method='otsu'):
    """Extract signal from specific tissue regions rather than whole image average"""
    if method == 'otsu' and len(volumes_list) > 0 and SKIMAGE_AVAILABLE:
        try:
            # Use Otsu's thresholding to identify tissue regions
            base_volume = volumes_list[0]
            if base_volume.ndim == 3:
                base_volume = base_volume[base_volume.shape[0] // 2]  # Use middle slice
            
            # Apply Otsu's method
            thresh = threshold_otsu(base_volume)
            binary = closing(base_volume > thresh, square(3))
            
            # Calculate signal only from tissue regions
            tissue_signals = []
            for vol in volumes_list:
                if vol.ndim == 3:
                    # For 3D volumes, use the same slice as the segmentation
                    mid_slice = vol[vol.shape[0] // 2]
                    tissue_signal = mid_slice[binary].mean()
                else:
                    tissue_signal = vol[binary].mean()
                tissue_signals.append(tissue_signal)
            
            return np.array(tissue_signals)
            
        except Exception as e:
            st.warning(f"Tissue segmentation failed: {e}. Using whole image average.")
            return compute_timeseries_from_volume_list(volumes_list, mask=None)
    
    else:
        if not SKIMAGE_AVAILABLE:
            st.warning("scikit-image not available. Using whole image average.")
        # Fallback to whole image average
        return compute_timeseries_from_volume_list(volumes_list, mask=None)


def estimate_relaxation_times(tvals, signal):
    """Estimate initial T1 and T2 values from signal decay to set appropriate ranges"""
    # Fit exponential decay to estimate T2
    try:
        # Simple exponential fit: S(t) = A * exp(-t/T2) + C
        def exp_decay(t, A, T2, C):
            return A * np.exp(-t/T2) + C
        
        # Initial guesses
        A0 = signal[0] - signal[-1]
        T20 = np.median(tvals) / 2
        C0 = signal[-1]
        
        # Fit curve
        popt, _ = curve_fit(exp_decay, tvals, signal, p0=[A0, T20, C0], maxfev=5000)
        A_est, T2_est, C_est = popt
        
        # Set T2 range based on estimate
        T2_min_est = max(0.001, T2_est / 10)
        T2_max_est = min(5.0, T2_est * 10)
        
        # For T1, we need a different approach since we don't have recovery data
        # Use typical T1/T2 ratios for biological tissues (3-10x)
        T1_min_est = max(0.01, T2_est * 3)
        T1_max_est = min(10.0, T2_est * 10)
        
        return T1_min_est, T1_max_est, T2_min_est, T2_max_est
        
    except Exception as e:
        st.warning(f"Exponential fit failed: {e}. Using default ranges.")
        return 0.05, 5.0, 0.01, 1.0  # Default ranges
...
# Add auto-range estimation option
auto_range = st.sidebar.checkbox('Auto-estimate T1/T2 ranges from data', value=True)

# Default values for manual range inputs
T1_min_default = 0.05
T1_max_default = 5.0
T2_min_default = 0.01
T2_max_default = 1.0
...
if not auto_range:
    T1_min = st.sidebar.number_input('T1 min (s)', min_value=0.001, value=T1_min_default, format='%f')
    T1_max = st.sidebar.number_input('T1 max (s)', min_value=0.01, value=T1_max_default, format='%f')
    T2_min = st.sidebar.number_input('T2 min (s)', min_value=0.001, value=T2_min_default, format='%f')
    T2_max = st.sidebar.number_input('T2 max (s)', min_value=0.001, value=T2_max_default, format='%f')
else:
    # Placeholder values that will be replaced after data processing
    T1_min = T1_min_default
    T1_max = T1_max_default
    T2_min = T2_min_default
    T2_max = T2_max_default
...
# Add tissue extraction option
tissue_extraction = st.sidebar.checkbox('Extract signal from tissue regions (Otsu)', value=True)
...
if run_button:
    if not dicom_dir:
        ...
        try:
            ...
            # Extract signal with tissue segmentation if enabled
            if tissue_extraction and not mask:
                S = extract_tissue_signal(volumes_list, method='otsu')
            else:
            ...
            # Auto-estimate T1/T2 ranges if enabled
            if auto_range:
                T1_min_est, T1_max_est, T2_min_est, T2_max_est = estimate_relaxation_times(tvals, S)
                T1_min = T1_min_est
                T1_max = T1_max_est
                T2_min = T2_min_est
                T2_max = T2_max_est
                st.write(f"Auto-estimated T1 range: {T1_min:.4f} - {T1_max:.4f} s")
                st.write(f"Auto-estimated T2 range: {T2_min:.4f} - {T2_max:.4f} s")
...
st.sidebar.write('scikit-image available:', SKIMAGE_AVAILABLE)
